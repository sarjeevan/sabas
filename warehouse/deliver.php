...
<body>
  ...
  $sql = "SELECT intend_items.ID,products.product_name,intend_items.product_quantity
  ...
  if (isset($_POST['submit_btn'])) {
   // var_dump($_POST);
    ...
    $sql1 = "UPDATE `intend` SET `status` = 'deliver',`deliver_date` =now(),`delivered_by` =$user_id   WHERE `intend`.`ID` = $id;";
    mysqli_query($conn, $sql1);
    foreach ($d_quantity as $key => $value) {
      $sql2 = "UPDATE intend_items SET deliver_quantity=$value  WHERE ID=$key";
      mysqli_query($conn, $sql2);


    }
    header("Location:viewintend.php?ID=$id");
  }
        ...
        if ($count > 0) {
          ...
          for ($i = 0; $i < $count; $i++) {
            ...
            <tr>
              ...
              <td class="text-center ">
                <input type="number" name="d_quantity[<?php echo $row['ID'] ?>]" id="<?php echo $row['ID'] ?>" />
              </td>
